                    ...
                    <div className="flex-1 flex flex-col relative w-full">
                        ...
                        <div className="flex gap-2 mb-3 flex-wrap">
                            {project.category.slice(0, 2).map((cat) => (
                                <Badge key={cat} variant="secondary" className="bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-neutral-500 dark:text-gray-400 text-[10px] font-normal border-none">
                                    {cat}
                                </Badge>
                            ))}
                        </div>
                        ...
                        <Link href={`/project/${project.id}`} className="block group/title">
                            <h3 className="text-xl font-bold mb-3 text-neutral-900 dark:text-white group-hover/title:text-blue-600 dark:group-hover/title:text-blue-400 transition-colors line-clamp-2 leading-tight">
                                {project.video_title}
                            </h3>
                        </Link>

                        <p className="text-neutral-600 dark:text-gray-400 text-sm mb-6 line-clamp-2 leading-relaxed">
                        ...
                        <div className="mt-auto pt-5 border-t border-black/5 dark:border-white/5 flex items-center justify-between">
                            <div className="flex items-center space-x-3">
                                <div className="w-8 h-8 rounded-full overflow-hidden border border-black/10 dark:border-white/10 bg-black/5 dark:bg-white/5 p-1">
                                    <Image
                                ...
                                <div className="flex flex-col">
                                    <span className="text-xs font-medium text-neutral-900 dark:text-white line-clamp-1 max-w-[100px] truncate">{project.client_name}</span>
                            ...
                            <div className="flex items-center gap-2">
                                <Link href={`/project/${project.id}`}>
                                    <Button
                                        size="sm"
                                        variant="ghost"
                                        className="h-8 px-5 text-xs font-medium text-neutral-900 dark:text-white bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-full shadow-[0_4px_12px_rgba(0,0,0,0.02)] dark:shadow-[0_4px_20px_rgba(255,255,255,0.1)] hover:shadow-[0_4px_20px_rgba(0,0,0,0.05)] dark:hover:shadow-[0_4px_20px_rgba(255,255,255,0.2)] hover:bg-black/10 dark:hover:bg-white/10 hover:border-black/20 dark:hover:border-white/20 transition-all duration-300 backdrop-blur-xl cursor-pointer"
                                    >
                                        Details
                                    </Button>
                                </Link>
